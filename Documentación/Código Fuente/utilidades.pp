...
TArchivo=FILE OF TDatosArchivo;
    ...
    vJobs = array [0..19] of datos;  // Vector para Jobs
   ...
   // puntero = cardinal;
	...
	var	stdOutPut: Text;			// Archivo de redirección de la salida estándar.
        ...
        punt: integer;           // Tamanio de vector Jobs.
        vector : vJobs;
        registro: datos;
        ruta: string;
...
//  UTILIDADES
    ...
    function buscarPrioridad(buscado:string):boolean;                   // Funcion para buscar el proceso con la prioridad mas alta
    function buscar2daPrioridad:integer;                                // FUncion parabuscar el proceso con la segunda prioridad mas alta
    function pidCoinciden(i:integer):boolean;                           // Funcion para determinar si los pid coinciden para ejecutar un comando
    function buscarPid(h:string):integer;                               // Funcion que devuelve el pid de un proceso
...
function buscarPid(h:string):integer;
var i: integer;
begin
for i:=0 to 19 do
if vector[i].prioridad = h then
buscarPid:=vector[i].pid;
end;

function pidCoinciden(i:integer):boolean;
var k: integer;
begin
pidCoinciden:=false;
for k:= 0 to 19 do
begin
if vector[k].pid= i then
pidCoinciden:=true;
end;
end;
